...
const AboutUs = () => {
  ...
  const timerRef = useRef(null); // Image navigation, Timer, Popup, Swipe, and useEffect
  ...
  return (
    <>
          ...
          <div className="w-full flex justify-between items-center mt-4">
            <div className="flex gap-2">
              {images.map((_, index) => (
                <div
                  key={index}
                  className={`w-6 sm:w-9 h-0.5 rounded-sm ${
      ...
      {popupOpen && (
        ...
        >
          <div
            className={`relative w-21/24 max-w-7xl max-h-[86vh] rounded-2xl bg-[rgba(214,196,196,0.56)] flex items-center justify-center overflow-hidden
              ${isClosing ? "animate-popupFadeReverse" : "animate-popupFade"}`}
          ...
          >
            <button
              className="absolute top-3 right-5 text-2xl font-bold text-white hover:text-[#bbff00] transition-transform hover:scale-[1.025] cursor-pointer z-1000 "
              onClick={closePopup}
            >
              &times;
            </button>
            <img
              ...
              className={`w-full h-auto object-contain rounded-2xl transition-opacity duration-300 ease-in-out ${
                fade
                  ? "opacity-0 scale-105"
                  : "opacity-100 scale-100 animate-fadeIn"
              }`}
            ...
            <button
              className="absolute top-1/2 left-3 -translate-y-1/2 bg-transparent border-none rounded-full text-4xl sm:text-5xl p-2 sm:p-3 text-white cursor-pointer transition-transform hover:scale-[1.025] hover:text-[#bbff00]"
              onClick={goToPreviousImage}
            >
              &#8592;
            </button>
            <button
              className="absolute top-1/2 right-3 -translate-y-1/2 bg-transparent border-none rounded-full text-4xl sm:text-5xl p-2 sm:p-3 text-white cursor-pointer transition-transform hover:scale-[1.025] hover:text-[#bbff00]"
              onClick={goToNextImage}
            >
              &#8594;
            </button>
            {/* Popup Indicators - for smaller gap on mobile */}       
