  ...
  return (
    <>
      <div className="absolute w-full z-20 flex justify-between items-center px-8 md:px-4 lg:px-40 py-4 bg-black">
        ...
        >
          {isOpen ? (
            ...
            <FontAwesomeIcon
              icon={faXmark}
              className="text-white bg-black -m-2 p-1 text-2xl"
            />
          ) : (
      ...
      <div
        ...
        } bg-[#00000025] w-full h-screen left-0 z-100 flex flex-col absolute md:hidden font-bold text-[1.2rem]  font-weight: 600  gap-12 font-poppins text-white text-sm uppercase transition-all duration-200`}
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="absolute px-10 right-0 top-19 py-7 flex flex-col gap-5 text-right h-fit w-full bg-[#000000ab]">
          <h3 className="">
            <a
              href="#advantages"
              className="hover:text-[#b1f100] transition-colors duration-200 ease-in-out"
